...
async function ensureApplyNowContent() {
  let doc = await ApplyNowContent.findOne();
  if (!doc) {
    doc = await ApplyNowContent.create(defaultApplyNowContent);
  }
  return doc;
}
...
router.get('/', async (req, res) => {
  try {
    const doc = await ensureApplyNowContent();
    res.json(doc);
  } catch (err) {
...
router.put('/', protect, async (req, res) => {
  try {
    const updated = await ApplyNowContent.findOneAndUpdate({}, req.body, {
      new: true,
      upsert: true,
      runValidators: true,
